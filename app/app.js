...
(function () {
  ...
  // ——— Объявление «связи с сервером нет» ——————————————————————
  // Прецедент 14.08.2026: сервер лёг (закрылся платёжный аккаунт хостинга), а
  // человек в Терминале видел только «Повторить» — куда идти с заявкой, не
  // говорил никто. Поясок в разметке ждёт наготове; поднимаем его при первой
  // же неудаче запроса и опускаем при первом успешном ответе. Формулировка
  // намеренно нейтральная («связи нет», не «сервер упал»): та же ошибка
  // случается и когда пропал интернет у самого человека.
  function outage(show) {
    var el = $('outageBanner');
    if (!el) return;
    el.hidden = !show;
  }
  ...
  function api(path, opts) {
    ...
    return fetchT(API_BASE + path, init).then(function (r) {
      return r.text().then(function (t) {
        ...
        if (!r.ok) {
          // 5xx — сервера фактически нет (контейнер не поднялся, шлюз молчит).
          // Прикладные отказы (401, 403, 409) сервер выдаёт сам и осмысленно —
          // поднимать на них объявление о недоступности было бы ложью.
          outage(r.status >= 500);
        ...
        outage(false);   // сервер ответил по делу — объявление снимаем
        if (!data) throw new Error('Сервер вернул пустой ответ.');
        return data;
      });
    }, function () {
      outage(true);      // до сервера не достучались вовсе
  ...
  /* ══════════════════════════════════════════════════════════════════
     ...
     Раздел А — та же лента, что была в «Разборах» (тот же код и те же id).
     ══════════════════════════════════════════════════════════════════ */
  ...
  // контрактные поля reading_url (JSON для Разборов) и pages_url (статическая
  // страница Pages). Склеивать пути из слага руками нельзя: сменит генератор
  // раскладку — Разборы начнут молча 404-ить, хотя правильный адрес лежит в ленте.
      ...
      .catch(function (err) {
        ...
        showState(stateEl,
          ...
          (known
            ...
            : 'Такого разбора в Разборах нет — возможно, ссылка устарела или в ней опечатка.' +
              '<br><a class="btn btn-ghost" href="' + polygonHash('razbory', null) + '">В Полигон</a>'), true);
